const textbook = {
  chapters: [
    {
      id: 1,
      name: 'Chapter 1: Thinking Like a Linguist',
      subchapters: [
        {
          id: 1,
          name: '1.1: Linguistics is Science',
          paragraphs: [
            {
              id: 1,
              text:
                'This unit introduces the idea of thinking scientifically about language by making empirical observations rather than judgments of correctness.',
            },
            {
              id: 2,
              text:
                'Linguistics is one of those subjects that not many people have heard of, so you might well be wondering exactly what it is.',
            },
            {
              id: 2,
              text:
                'The passage experienced a surge in popularity during the 1960s when Letraset used it on their dry-transfer sheets, and again during the 90s as desktop publishers bundled the text with their software. ',
            },
            {
              id: 2,
              text:
                'HELLO HELLO HELLO HELLO HELLO HELLO HELLO HELLO HELLO HELLO HELLO HELLO HELLO HELLO HELLO HELLO HELLO HELLO HELLO HELLO HELLO HELLO HELLO HELLO ',
            },
          ],
        },
        {
          id: 2,
          name: '1.2: Mental Grammar',
          paragraphs: [
            {
              id: 1,
              text:
                'Linguistics is part of the broad field of cognitive science, which studies the human mind. Linguistics focus specifically on the mental grammar: the system that all speakers of a language have in their minds, which allows them to understand each other. The mental grammar of every language includes phonetics, phonology, morphology, syntax, and semantics.',
            },
            {
              id: 2,
              text:
                'The purpose of lorem ipsum is to create a natural looking block of text (sentence, paragraph, page, etc.) that doesnt distract from the layout. A practice not without',
            },
            {
              id: 2,
              text:
                'Richard McClintock, a Latin scholar from Hampden-Sydney College, is credited with discovering the source behind the ubiquitous filler text. In seeing a sample of lorem ipsum',
            },
          ],
        },
        {
          id: 2,
          name: '1.3: The Art of Writing',
          paragraphs: [
            {
              id: 1,
              text:
                'Elit velit excepteur eiusmod nostrud labore id sint exercitation. Cillum veniam duis officia nostrud in incididunt duis nulla esse commodo esse ut pariatur amet. Excepteur officia officia incididunt officia aliqua consequat pariatur labore duis consequat enim.',
            },
            {
              id: 2,
              text:
                'Ut sit sunt Lorem dolor occaecat voluptate magna Lorem sint laborum enim. Consectetur nulla quis magna velit Lorem. Consequat esse consectetur aute ipsum nisi enim aute id ullamco anim laborum fugiat irure.',
            },
            {
              id: 2,
              text:
                'Minim ullamco exercitation velit laboris et eiusmod Lorem. Nulla non mollit quis sunt aliqua. Do tempor cupidatat ea et ut dolore. Dolore proident reprehenderit voluptate in duis sit proident proident. Enim minim labore adipisicing enim velit cillum.',
            },
          ],
        },
      ],
    },
    {
      id: 2,
      name: 'Chapter 2: Producing Speech Sounds',
      subchapters: [
        {
          id: 1,
          name: '2.1: Understanding phonetics',
          paragraphs: [
            {
              id: 1,
              text:
                'Phonetics studies human speech. Speech is produced by bringing air from the lungs to the larynx (respiration), where the vocal folds may be held open to allow the air to pass through or may vibrate to make a sound (phonation). The airflow from the lungs is then shaped by the articulators in the mouth and nose (articulation).',
            },
            {
              id: 2,
              text:
                'The field of phonetics studies the sounds of human speech.  When we study speech sounds we can consider them from two angles.  Acoustic phonetics,  in addition to being part of linguistics, is also a branch of physics.  It’s concerned with the physical, acoustic properties of the sound waves that we produce.  We’ll talk some about the acoustics of speech sounds, but we’re primarily interested in articulatory phonetics, that is, how we humans use our bodies to produce speech sounds. Producing speech needs three mechanisms.',
            },
            {
              id: 2,
              text:
                'McClintocks eye for detail certainly helped narrow the whereabouts of lorem ipsums origin, however, the “how and when” still remain something of a mystery, with competing theories and timelines.',
            },
          ],
        },
        {
          id: 2,
          name: '2.2: Articulators',
          paragraphs: [
            {
              id: 1,
              text:
                'Speaking involves controlling parts of the mouth and nose to shape the air that comes from the lungs. This unit introduces the names and locations of the articulators that are used to produce the sounds of English: the tongue, lips and teeth, the alveolar ridge, the palate, the velum, and the nasal cavity.',
            },
            {
              id: 1,
              text:
                'Nor is there anyone who loves or pursues or desires to obtain pain of itself, because it is pain, but occasionally circumstances occur in which toil and pain can procure him some great pleasure',
            },
            {
              id: 1,
              text:
                'So how did the classical Latin become so incoherent? According to McClintock, a 15th century typesetter likely scrambled part of Ciceros De Finibus in order to provide placeholder text to mockup various fonts for a type specimen book.',
            },
          ],
        },
        {
          id: 1,
          name: '2.3: Vocal Chord Anatomy',
          paragraphs: [
            {
              id: 1,
              text:
                'Cillum est laborum est id do in in. Sunt labore Lorem duis est ullamco magna sit cillum sunt ut ipsum culpa. Proident excepteur culpa esse sunt laboris dolor reprehenderit enim et non ut. Do mollit dolore occaecat eiusmod sunt ullamco velit aliquip pariatur officia sint in.',
            },
            {
              id: 2,
              text:
                'Nulla aliquip culpa exercitation eiusmod fugiat non fugiat sunt commodo officia adipisicing. Qui excepteur nisi fugiat aliquip id culpa pariatur qui. Aliquip Lorem culpa nostrud do esse ullamco sunt reprehenderit consectetur esse culpa quis. Ad ipsum non nulla incididunt in id nulla dolore. Anim consectetur esse fugiat veniam amet culpa ex.',
            },
            {
              id: 2,
              text:
                'Laboris voluptate ullamco eu eiusmod amet et minim qui cupidatat. Et nulla culpa voluptate sint voluptate. Et reprehenderit deserunt pariatur duis incididunt ipsum est deserunt qui consequat laborum.',
            },
          ],
        },
      ],
    },
  ],
  name: 'Essentials of Linguistics',
};

export default textbook;
